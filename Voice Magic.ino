...
const uint16_t PIN2 = 7;  
const uint16_t NUM_PIXELS = 120;    // 点灯させるLEDの数を指定
const uint16_t NUM_PIXELS2 = 18;    // 点灯させるLEDの数を指定
...
int hueBottom2 = 10;               // グラデーションの下端（0〜100）
int hueTop2 = 80; 
...
const int LENGTH_NUM = 4;         // 検出する音の長さ
...
SpresenseNeoPixel<PIN2, NUM_PIXELS2> neopixel2;
char pixelArry[NUM_PIXELS];       // LEDテープを制御するための配列
char pixelArry2[NUM_PIXELS];
int col[3];   
int col2[3];                    // 色を格納する配列
int intervalCount1 = 0;           // LEDの処理待ちインターバル用のカウント
bool intervalFlg = false;         // LEDの処理待ちインターバル用のフラグ
int intervalCount3 = 0;           // LEDの処理待ちインターバル用のカウント
bool intervalFlg2 = false; 
...
void setup()
{
  ...
  //theAudio->setRecorderMode(AS_SETRECDR_STS_INPUTDEVICE_MIC, 210); // 入力ゲインを上げる場合はこちら
...
void ledControl2() {
  ...
  if(!intervalFlg2) {
    ...
    // アニメーションさせるために、配列をシフトさせる
    memmove(&pixelArry2[1], &pixelArry2[0], 49);

    // 取得した音量から、LEDの明るさにマッピング
    int tmpBright = map(foundVolume, 30, volumeInput2+30, 0, 9);
    // LEDの明るさをLEDの配列の先頭に格納
    pixelArry2[0] = tmpBright;


    // LEDの配列をひとつずつ取り出し、LEDの色を設定する
    for (int i=0; i<NUM_PIXELS2; i++) {
      int tmpPixel = pixelArry2[i];
      if(tmpPixel > 0) {
        
        // LEDの位置によって色をかえる
        hue2 = map(i, 0, NUM_PIXELS2-1, hueBottom2, hue2) / 100.0;
        float workBright = pixelArry2[i] / 10.0;
        hsv2rgb(hue2, saturation2, workBright, col2);
        neopixel2.set(i, col2[0], col2[1], col2[2]);    // LEDの色をRGBで指定
      } else {
        neopixel2.set(i, 0, 0, 0);   // 値が0の場合は消灯（黒を指定）
      }
